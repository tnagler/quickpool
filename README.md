...
* [`parallel_for_each(x, f)`](https://tnagler.github.io/quickpool/namespacequickpool.html#aeb91fe18664b8d06523aba081174abe3) runs `f(*it)` for all random-access iterators `std::begin(x) <= it < std::end(x)`.
...
Existing sequential loops are easy to parallelize. `parallel_for_each()` requires
a random-access range, such as `std::vector`:
```cpp
